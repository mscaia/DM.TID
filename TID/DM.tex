...
\begin{figure}[htbp]
  ...
  \resizebox{\textwidth}{!}{
  \begin{tikzpicture}[
    every node/.style={draw, rectangle, rounded corners, align=center, minimum size=6mm, font=\scriptsize},
  ...
  \node {Chapeau}
    child {node {Pointu}
        child {node {Non comestible}}
    }
    child {node {Arrondi}
        child {node {Couleur}
          child {node {Brun}
              child {node {Comestible}}
          }
          child {node {Jaune}
              child {node {Comestible} edge from parent node[left] {épais}}
              child {node {Non comestible} edge from parent node[right] {fin}}
          }
        }
    }
    child {node {Plat}
        child {node {Couleur}
          ...
          child {node {Jaune}
              child {node {Comestible} edge from parent node[left] {Champignon d}}
              child {node {Non comestible} edge from parent node[right] {Champignon j}}
          }
        }
    };
  \end{tikzpicture}
